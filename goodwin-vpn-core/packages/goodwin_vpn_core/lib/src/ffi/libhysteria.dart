...
import 'libxray.dart' show XrayResponse;
...
/// FFI wrapper around `libhysteria` c-shared (`Start` / `Stop` / `IsStarted`).
class LibHysteria {
  LibHysteria._(this._start, this._stop, this._isStarted, this._free, this._version);
  ...
  static LibHysteria open(String path) {
    final lib = DynamicLibrary.open(path);
    return LibHysteria._(
      ...
      lib.lookupFunction<_VersionC, _VersionDart>('GetHysteriaVersion'),
    );
  }
  ...
  XrayResponse _read(Pointer<Void> ptr) {
    if (ptr == nullptr) {
      throw StateError('null response from libhysteria');
    }
    try {
